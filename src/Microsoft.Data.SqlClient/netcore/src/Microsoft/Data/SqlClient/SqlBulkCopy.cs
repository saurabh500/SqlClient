...
namespace Microsoft.Data.SqlClient
{
    ...
    public sealed class SqlBulkCopy : IDisposable
    {
        ...
        private SourceColumnMetadata GetColumnMetadata(int ordinal)
        {
            ...
            else if ((_enableStreaming) && (metadata.length == MAX_LENGTH) || (_enableStreaming && metadata.type == SqlDbTypeExtensions.Json))
            {
                isSqlType = false;
            ...
            try
            {
                ...
                if (resultTask != null)
                {
                    ...
                    return resultTask.ContinueWith(
                        static (Task t, object state) =>
                        {
                            SqlBulkCopy sqlBulkCopy = (SqlBulkCopy)state;
                            try
                            {
                                sqlBulkCopy.AbortTransaction(); // If there is one, on success transactions will be committed.
                            }
                            finally
                            {
