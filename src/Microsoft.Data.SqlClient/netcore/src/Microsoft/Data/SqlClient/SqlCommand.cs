            ...
            returnTask = Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    ReadDescribeEncryptionParameterResults(describeParameterEncryptionDataReader,
                        describeParameterEncryptionRpcOriginalRpcMap, inRetry);
#if DEBUG
                    // Failpoint to force the thread to halt to simulate cancellation of SqlCommand.
                    if (_sleepAfterReadDescribeEncryptionParameterResults)
                    {
            ...
            if (describeParameterEncryptionTask != null)
            {
                ...
                AsyncHelper.ContinueTaskWithState(describeParameterEncryptionTask, completion, this,
                    (object state) =>
                    {
                        ...
                        command.RunExecuteReaderTds(cmdBehavior, runBehavior, returnStream, isAsync, TdsParserStaticMethods.GetRemainingTimeout(timeout, parameterEncryptionStart), out subTask, asyncWrite, inRetry, ds);
                        if (subTask == null)
                        {
                            completion.SetResult(null);
                        }
            ...
            if (null != thisParam)
            {
                    ...
                    thisParam.Direction == ParameterDirection.ReturnValue))
                {
                    ...
                    else
                    {
                        ...
                        TdsParserExtensions.GetNullSqlValue(buff, rec, SqlCommandColumnEncryptionSetting.Enabled, parser.Connection?.ConnectionOptions ?? null);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                else if (!mt.IsFixed && !mt.IsLong && mt.SqlDbType != SqlDbType.Timestamp && mt.SqlDbType != SqlDbType.Udt && SqlDbType.Structured != mt.SqlDbType)
                {
                    ...
                    if (mt.IsAnsiType)
                    {
                        ...
                        if (null != s)
                        {
                            int actualBytes = TdsParser.GetEncodingCharLength(s,
