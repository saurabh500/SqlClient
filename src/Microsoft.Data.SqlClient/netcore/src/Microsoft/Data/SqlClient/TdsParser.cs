...
namespace Microsoft.Data.SqlClient
{
    ...
    internal sealed partial class TdsParser
    {
        ...
        private static void FireInfoMessageEvent(SqlConnection connection, SqlCommand command, TdsParserStateObject stateObj, SqlError error,
        ...
        internal static void RollbackOrphanedAPITransactions(SqlInternalTransaction CurrentTransaction)
        {
        ...
        internal void ThrowExceptionAndWarning(TdsParserStateObject stateObj, SqlCommand command = null, bool callerHasConnectionLock = false, bool asyncClose = false)
        {
            ...
            if (breakConnection)
            {
                if ((_state == TdsParserState.OpenNotLoggedIn) &&
                    (_connHandler.ConnectionOptions.MultiSubnetFailover || _loginWithFailover) && (temp.Count == 1) && 
        ...
        internal bool Run(RunBehavior runBehavior, SqlCommand cmdHandler, SqlDataReader dataStream, 
            ...
            SqlInternalConnectionTds _connHandler)
        {
            ...
            {
                stateObj._syncOverAsync = true;
                bool result = TryRun(runBehavior, cmdHandler, dataStream, bulkCopyHandler, stateObj, _connHandler,
                    out bool dataReady);
            ...
            out bool dataReady)
        {
            ...
            do
            {
                ...
                if (!stateObj._accumulateInfoEvents && (stateObj._pendingInfoEvents != null))
                {
                    if (RunBehavior.Clean != (RunBehavior.Clean & runBehavior))
                    {
                        ...
                        if (connectionHandler != null)
                            connection = connectionHandler.Connection; // SqlInternalConnection holds the user connection object as a weak ref
                        ...
                        if ((connection != null) && connection.FireInfoMessageEventOnUserErrors)
                        {
                            ...
                            {
                                FireInfoMessageEvent(connection, cmdHandler, stateObj, error, _state, connectionHandler);
                            }
                        }
                        else
                            foreach (SqlError error in stateObj._pendingInfoEvents)
                ...
                if (!TdsParserExtensions.IsValidTdsToken(token))
                {
                    Debug.Fail($"unexpected token; token = {token,-2:X2}");
                    _state = TdsParserState.Broken;
                    connectionHandler.BreakConnection();
                ...
                int tokenLength;
                if (!TdsParserExtensions.TryGetTokenLength(token, stateObj, out tokenLength))
                {
                    return false;
                }
                    ...
                    case TdsEnums.SQLINFO:
                        {
                            ...
                            if (!TdsParser.TryProcessError(token, stateObj, cmdHandler, out SqlError error, _server))
                            {
                                return false;
                            }
                            ...
                            if (RunBehavior.Clean != (RunBehavior.Clean & runBehavior))
                            {
                                    ...
                                    (error.Class <= TdsEnums.MAX_USER_CORRECTABLE_ERROR_CLASS))
                                {
                                    ...
                                    TdsParser.FireInfoMessageEvent(connection, cmdHandler, stateObj, error, _state, connectionHandler);
                                }
                                else
                                {
                    ...
                    case TdsEnums.SQLCOLINFO:
                        {
                            if (null != dataStream)
                            {
                                ...
                                if (!TdsParser.TryProcessColInfo(dataStream.MetaData, dataStream, stateObj, out metaDataSet))
                                {
                                    return false;
                                }
                    ...
                    case TdsEnums.SQLENVCHANGE:
                        {
                            ...
                            if (!TdsParser.TryProcessEnvChange(tokenLength, 
                                stateObj, 
                                _physicalStateObj, 
                            ...
                            while (env != null)
                            {
                                if (!connectionHandler.IgnoreEnvChange)
                                {
                                    switch (env._type)
                                    {
                                        ...
                                        default:
                                            connectionHandler.OnEnvChange(env);
                                            break;
                                    }
                                }
                    ...
                    case TdsEnums.SQLLOGINACK:
                        {
                            ...
                            connectionHandler.OnLoginAck(ack);
                            break;
                        }
                    ...
                    case TdsEnums.SQLFEDAUTHINFO:
                        {
                            ...
                            connectionHandler.OnFedAuthInfo(info);
                            break;
                        }
                    case TdsEnums.SQLSESSIONSTATE:
                        {
                            if (!TdsParserExtensions.TryProcessSessionState(stateObj, tokenLength, connectionHandler))
                            {
                                return false;
                            }
                            break;
                    ...
                    case TdsEnums.SQLCOLMETADATA:
                        {
                            ...
                            if (TdsEnums.SQLDATACLASSIFICATION == peekedToken)
                            {
                                ...
                                if (!TryProcessDataClassification(
                                    stateObj, 
                                    out SensitivityClassification sensitivityClassification, 
                                ...
                                if (dataStream != null)
                                {
                                    dataStream.SensitivityClassification = sensitivityClassification;
                                }
                            ...
                            if (null != dataStream)
                            {
                                if (!dataStream.TrySetMetaData(stateObj._cleanupMetaData, 
                                    (TdsEnums.SQLTABNAME == peekedToken || TdsEnums.SQLCOLINFO == peekedToken)))
                                {
                                    return false;
                                }
                            }
                    ...
                    case TdsEnums.SQLRETURNVALUE:
                        {
                            ...
                            if (!TryProcessReturnValue(tokenLength,
                                stateObj,
                                out returnValue,
                                cmdHandler?.ColumnEncryptionSetting ?? SqlCommandColumnEncryptionSetting.UseConnectionSetting,
                    ...
                    case TdsEnums.SQLTABNAME:
                        {
                            if (null != dataStream)
                            {
                                ...
                                if (!TdsParserExtensions.TryProcessTableName(tokenLength, stateObj, out tableNames))
                                {
                                    return false;
                                }
            ...
            if ((stateObj.HasPendingData) && (!dataReady))
            {
                ...
                Debug.Assert(TdsParserExtensions.IsValidTdsToken(token), $"DataReady is false, but next token is not valid: {token,-2:X2}");
            }
#endif
            ...
            while (tokenLength > processedLength)
            {
                ...
                switch (env._type)
                {
                    ...
                    case TdsEnums.ENV_LANG:
                        if (!TdsParserExtensions.TryReadTwoStringFields(env, stateObj))
                        {
                            return false;
                        }
                        break;
                    ...
                    case TdsEnums.ENV_SPRESETCONNECTIONACK:
                        if (!TdsParserExtensions.TryReadTwoBinaryFields(env, stateObj))
                        {
                            return false;
                        }
                        break;
        ...
        internal static bool TryReadByteString(TdsParserStateObject stateObj, out string value)
        {
            value = string.Empty;
        ...
        private static bool TryReadSensitivityLabel(TdsParserStateObject stateObj, out string label, out string id)
        {
            label = string.Empty;
            id = string.Empty;

            if (!TdsParser.TryReadByteString(stateObj, out label))
            {
                return false;
            }

            if (!TdsParser.TryReadByteString(stateObj, out id))
            {
                return false;
            }
        ...
        private static bool TryReadSensitivityInformationType(TdsParserStateObject stateObj, out string informationType, out string id)
        {
        ...
        private static bool TryProcessDataClassification(TdsParserStateObject stateObj,
            out SensitivityClassification sensitivityClassification,
        ...
        private static bool TryProcessResColSrcs(TdsParserStateObject stateObj, int tokenLength)
        {
            if (!stateObj.TrySkipBytes(tokenLength))
            {
                return false;
        ...
        internal static bool TryProcessError(byte token, TdsParserStateObject stateObj, SqlCommand command, out SqlError error, string _server)
        {
            ushort shortLen;
            byte byteLen;
            int number;
        ...
        internal static bool TryProcessReturnValue(int length, 
            TdsParserStateObject stateObj, 
            ...
            Action<TdsParserStateObject> ThrowUnsupportedCollationEncountered)
        {
            ...
            else if (TdsParserExtensions.IsVarTimeTds(tdsType))
            ...
            {
                if (!TdsParserExtensions.TryGetTokenLength(tdsType, stateObj, out tdsLen))
                {
                    return false;
                }
            }
            ...
            {
                if (!TdsParserExtensions.TryProcessUDTMetaData((SqlMetaDataPriv)rec, stateObj))
                {
                    return false;
                }
            }
        ...
        internal void DrainData(TdsParserStateObject stateObj)
        {
            try
            {
                ...
                Run(RunBehavior.Clean, null, null, null, stateObj, _connHandler);
            }
            catch
            {
        ...
        private void WriteUDTMetaData(object value, string database, string schema, string type, TdsParserStateObject stateObj)
        {
            ...
            else
            {
                ...
                TdsParserExtensions.WriteString(database, stateObj);
            }
            ...
            else
            {
                ...
                TdsParserExtensions.WriteString(schema, stateObj);
            }
            ...
            else
            {
                ...
                TdsParserExtensions.WriteString(type, stateObj);
            }
        }
        ...
        private static bool TryProcessColInfo(_SqlMetaDataSet columns, SqlDataReader reader, TdsParserStateObject stateObj, out _SqlMetaDataSet metaData)
        {
        ...
        private static bool TryProcessColumnHeaderNoNBC(SqlMetaDataPriv col, TdsParserStateObject stateObj, out bool isNull, out ulong length)
        {
        ...
        private static bool TryReadSqlStringValue(SqlBuffer value, byte type, int length, Encoding encoding, 
            bool isPlp, 
            TdsParserStateObject stateObj,
        ...
        /// </summary>
        internal static bool DeserializeUnencryptedValue(SqlBuffer value, 
        ...
        internal static bool TryReadSqlValue(SqlBuffer value, 
            SqlMetaDataPriv md, 
            int length, 
            TdsParserStateObject stateObj, 
            ...
            SqlCommand command = null)
        {
            ...
            switch (tdsType)
            {
                case TdsEnums.SQLDECIMALN:
                case TdsEnums.SQLNUMERICN:
                    if (!TdsParserExtensions.TryReadSqlDecimal(value, length, md.precision, md.scale, stateObj))
                    {
                        return false;
                    }
                    break;
        ...
        private static bool TryReadSqlDateTime(SqlBuffer value, byte tdsType, int length, byte scale, TdsParserStateObject stateObj)
        {
        ...
        internal static bool TryReadSqlValueInternal(SqlBuffer value, 
            byte tdsType, 
            int length, 
            TdsParserStateObject stateObj,
            Encoding defaultEncoding,
                ...
                case TdsEnums.SQLNUMERICN:
                    {
                        ...
                        if (!TdsParserExtensions.TryReadSqlDecimal(value, TdsEnums.MAX_NUMERIC_LEN, precision, scale, stateObj))
                        {
                            return false;
                        }
                        break;
            ...
            bool canAccumulate = true)
        {
            ...
            if (ADP.IsNull(value))
            {
                TdsParserExtensions.WriteInt(TdsEnums.FIXEDNULL, stateObj); //maxlen
                TdsParserExtensions.WriteInt(TdsEnums.FIXEDNULL, stateObj); //actuallen
                return null;
            }
            ...
            TdsParserExtensions.WriteInt(TdsEnums.SQLVARIANT_SIZE + mt.PropBytes + length, stateObj); // maxLen
            TdsParserExtensions.WriteInt(TdsEnums.SQLVARIANT_SIZE + mt.PropBytes + length, stateObj); // actualLen
            ...
            switch (mt.TDSType)
            {
                ...
                case TdsEnums.SQLNVARCHAR:
                    {
                        ...
                        length >>= 1;
                        return TdsParserExtensions.WriteString(s, length, offset, stateObj, canAccumulate);
                    }
                ...
                case TdsEnums.SQLTIME:
                    ...
                    TdsParserExtensions.WriteTime((TimeSpan)value, mt.Scale, length, stateObj);
                    break;

                case TdsEnums.SQLDATETIMEOFFSET:
                    stateObj.WriteByte(mt.Scale); //propbytes: scale
                    TdsParserExtensions.WriteDateTimeOffset((DateTimeOffset)value, mt.Scale, length, stateObj);
                    break;
            ...
            SqlInternalConnectionTds Connection, Action<TdsParserStateObject> ThrowUnsupportedCollationEncountered, bool canAccumulate = true)
        {
            ...
            {
                TdsParserExtensions.WriteInt(TdsEnums.FIXEDNULL, stateObj);
                return null;
            }
                ...
                case TdsEnums.SQLNVARCHAR:
                    {
                        ...
                        // string takes cchar, not cbyte so convert
                        length >>= 1;
                        return TdsParserExtensions.WriteString(s, length, 0, stateObj, canAccumulate);
                    }
        ...
        private static void WriteSqlMoney(SqlMoney value, int length, TdsParserStateObject stateObj)
        {
            ...
            if (length == 4)
            {
                ...
                TdsParserExtensions.WriteInt((int)l, stateObj);
            }
            else
            {
                TdsParserExtensions.WriteInt((int)(l >> 0x20), stateObj);
                TdsParserExtensions.WriteInt((int)l, stateObj);
            }
        }
        ...
        private void ProcessAttention(TdsParserStateObject stateObj)
        {
            ...
            try
            {
                ...
                Run(RunBehavior.Attention, null, null, null, stateObj, _connHandler);
            }
            catch (Exception e)
            {
        ...
        internal static void SendFedAuthToken(SqlFedAuthToken fedAuthToken, TdsParserStateObject _physicalStateObj)
        {
            ...
            TdsParserExtensions.WriteUnsignedInt((uint)accessToken.Length + sizeof(uint), _physicalStateObj);

            // Send length of token
            TdsParserExtensions.WriteUnsignedInt((uint)accessToken.Length, _physicalStateObj);
            ...
            try
            {
                ...
                switch (request)
                {
                    ...
                    case TdsEnums.TransactionManagerRequestType.Begin:
                        ...
                        TdsParserExtensions.WriteString(transactionName, stateObj);
                        break;
                ...
                {
                    Run(RunBehavior.UntilDone, null, null, null, stateObj, _connHandler);
                }
        ...
        internal Task TdsExecuteSQLBatch(string text, int timeout, SqlNotificationRequest notificationRequest, TdsParserStateObject stateObj, bool sync, bool callerHasConnectionLock = false, byte[] enclavePackage = null)
        {
            ...
            try
            {
                ...
                TdsParserExtensions.WriteString(text, text.Length, 0, stateObj);
        ...
        private Task TDSExecuteRPCAddParameter(TdsParserStateObject stateObj, SqlParameter param, MetaType mt, byte options, SqlCommand command, bool isAnonymous)
        {
            ...
            else
            {
                ...
                else if (mt.SqlDbType == SqlDbType.Udt)
                {
                    ...
                    if (!isNull)
                    {
                        TdsParserExtensions.WriteUnsignedLong((ulong)udtVal.Length, stateObj); // PLP length
                        if (udtVal.Length > 0)
                        { // Only write chunk length if its value is greater than 0
                            TdsParserExtensions.WriteInt(udtVal.Length, stateObj); // Chunk length
                            stateObj.WriteByteArray(udtVal, udtVal.Length, 0); // Value
                        }
                        TdsParserExtensions.WriteInt(0, stateObj); // Terminator
                    }
                    else
                    {
                        TdsParserExtensions.WriteUnsignedLong(TdsEnums.SQL_PLP_NULL, stateObj); // PLP Null.
                    }
            ...
            if (_is2005 && (mt.SqlDbType == SqlDbType.Xml))
            {
                    ...
                    !string.IsNullOrEmpty(param.XmlSchemaCollectionName))
                {
                    ...
                    if (!string.IsNullOrEmpty(param.XmlSchemaCollectionDatabase))
                    {
                        ...
                        TdsParserExtensions.WriteString(param.XmlSchemaCollectionDatabase, tempLen, 0, stateObj);
                    }
                    else
                    {
                    ...
                    if (!string.IsNullOrEmpty(param.XmlSchemaCollectionOwningSchema))
                    {
                        ...
                        stateObj.WriteByte((byte)(tempLen));
                        TdsParserExtensions.WriteString(param.XmlSchemaCollectionOwningSchema, tempLen, 0, stateObj);
                    }
                    else
                    {
        ...
        private static void WriteParameterName(string rawParameterName, TdsParserStateObject stateObj, bool isAnonymous)
        {
            ...
            if (!isAnonymous && !string.IsNullOrEmpty(rawParameterName))
            {
                ...
                if (writePrefix)
                {
                    TdsParserExtensions.WriteString("@", 1, 0, stateObj);
                }
                TdsParserExtensions.WriteString(rawParameterName, nameLength, 0, stateObj);
            }
            else
            {
                stateObj.WriteByte(0);
            ...
            SqlCollation defaultCollation)
        {
            switch (metaData.SqlDbType)
            {
                ...
                case SqlDbType.Variant:
                    ...
                    TdsParserExtensions.WriteInt(checked((int)metaData.MaxLength), stateObj);
                    break;
                case SqlDbType.Xml:
                    ...
                    else
                    {
                        ...
                        TdsParserExtensions.WriteIdentifier(metaData.TypeSpecificNamePart1, stateObj);
                        TdsParserExtensions.WriteIdentifier(metaData.TypeSpecificNamePart2, stateObj);
                        TdsParserExtensions.WriteIdentifierWithShortLength(metaData.TypeSpecificNamePart3, stateObj);
                    }
                    break;
                case SqlDbType.Udt:
                    ...
                    TdsParserExtensions.WriteIdentifier(metaData.TypeSpecificNamePart1, stateObj);
                    TdsParserExtensions.WriteIdentifier(metaData.TypeSpecificNamePart2, stateObj);
                    TdsParserExtensions.WriteIdentifier(metaData.TypeSpecificNamePart3, stateObj);
                    break;
        ...
        private static void WriteTvpTypeInfo(SmiExtendedMetaData metaData, TdsParserStateObject stateObj, SqlCollation defaultCollation)
        {
            ...
            {
                TdsParserExtensions.WriteUnsignedShort((ushort)TdsEnums.TVP_NOMETADATA_TOKEN, stateObj);
            }
            else
            {
                // COUNT of columns
                TdsParserExtensions.WriteUnsignedShort(checked((ushort)metaData.FieldMetaData.Count), stateObj);
            ...
            SqlCollation defaultCollation)
        {
            ...
            {
                TdsParserExtensions.WriteUnsignedInt(TdsEnums.SQLTIMESTAMP, stateObj);
            }
            else
            {
                TdsParserExtensions.WriteUnsignedInt(0, stateObj);
            }
            ...
            TdsParserExtensions.WriteIdentifier(null, stateObj);
        }
        ...
        internal void WriteEncryptionEntries(ref SqlTceCipherInfoTable cekTable, TdsParserStateObject stateObj)
        {
            ...
            {
                // Write Db ID
                TdsParserExtensions.WriteInt(cekTable[i].DatabaseId, stateObj);

                // Write Key ID
                TdsParserExtensions.WriteInt(cekTable[i].CekId, stateObj);

                // Write Key Version
                TdsParserExtensions.WriteInt(cekTable[i].CekVersion, stateObj);
        ...
        internal void WriteTceUserTypeAndTypeInfo(SqlMetaDataPriv mdPriv, TdsParserStateObject stateObj)
        {
            ...
            TdsParserExtensions.WriteInt(0x0, stateObj); // TODO: fix this- timestamp columns have 0x50 value here
            ...
            switch (mdPriv.type)
            {
                ...
                default:
                    ...
                    {
                        TdsParserExtensions.WriteUnsignedInt(mdPriv.collation._info, stateObj);
        ...
        internal void WriteCryptoMetadata(_SqlMetaData md, TdsParserStateObject stateObj)
        {
            ...
            if (TdsEnums.CustomCipherAlgorithmId == md.cipherMD.CipherAlgorithmId)
            {
                ...
                TdsParserExtensions.WriteString(md.cipherMD.CipherAlgorithmName, stateObj);
            }
        ...
        internal void WriteBulkCopyMetaData(_SqlMetaDataSet metadataCollection, int count, TdsParserStateObject stateObj)
        {
            ...
            for (int i = 0; i < metadataCollection.Length; i++)
            {
                if (metadataCollection[i] != null)
                {
                    ...
                    // read user type - 4 bytes 2005, 2 backwards
                    TdsParserExtensions.WriteInt(0x0, stateObj);
                    ...
                    switch (md.type)
                    {
                        ...
                        default:
                            ...
                            {
                                TdsParserExtensions.WriteUnsignedInt(md.collation._info, stateObj);
                    ...
                    TdsParserExtensions.WriteString(md.column, stateObj);
                }
            } // end for loop
        }
            ...
            TdsParser parser)
        {
            ...
            try
            {
                ...
                if (isNull)
                {
                    ...
                    {
                        TdsParserExtensions.WriteLong(unchecked((long)TdsEnums.SQL_PLP_NULL), stateObj);
                    }
                ...
                if (metatype.IsLong)
                {
                    switch (metatype.SqlDbType)
                    {
                        ...
                        case SqlDbType.Udt:
                            // plp data
                            TdsParserExtensions.WriteUnsignedLong(TdsEnums.SQL_PLP_UNKNOWNLEN, stateObj);
                            break;
                    }
                }
                else
        ...
        private static void WriteMarsHeaderData(TdsParserStateObject stateObj, SqlInternalTransaction transaction, long _retainedTransactionId)
        {
            ...
            {
                TdsParserExtensions.WriteLong(transaction.TransactionId, stateObj);
                TdsParserExtensions.WriteInt(stateObj.IncrementAndObtainOpenResultCount(transaction), stateObj);
            }
            else
            {
                // If no transaction, send over retained transaction descriptor (empty if none retained)
                TdsParserExtensions.WriteLong(_retainedTransactionId, stateObj);
                TdsParserExtensions.WriteInt(stateObj.IncrementAndObtainOpenResultCount(null), stateObj);
            }
        }
            ...
            bool isDataFeed)
        {
            if (type.IsPlp && ((actualLength > 0) || isDataFeed))
            {
                ...
                {
                    return AsyncHelper.CreateContinuationTask<int, TdsParserStateObject>(unterminatedWriteTask, TdsParserExtensions.WriteInt, 0, stateObj);
                }
            }
            else
            {
            ...
            switch (type.NullableType)
            {
                case TdsEnums.SQLFLTN:
                    if (type.FixedLength == 4)
                        TdsParserExtensions.WriteFloat(((SqlSingle)value).Value, stateObj);
                    else
                    {
                        Debug.Assert(type.FixedLength == 8, "Invalid length for SqlDouble type!");
                        TdsParserExtensions.WriteDouble(((SqlDouble)value).Value, stateObj);
                    }
                ...
                case TdsEnums.SQLIMAGE:
                    {
                        if (type.IsPlp)
                        {
                            TdsParserExtensions.WriteInt(actualLength, stateObj);               // chunk length
                        }
                ...
                case TdsEnums.SQLINTN:
                    ...
                    else
                    {
                        ...
                        TdsParserExtensions.WriteLong(((SqlInt64)value).Value, stateObj);
                    }
                ...
                case TdsEnums.SQLTEXT:
                    if (type.IsPlp)
                    {
                        TdsParserExtensions.WriteInt(codePageByteSize, stateObj);               // chunk length
                    }
                    if (value is SqlChars)
                    {
                        string sch = new string(((SqlChars)value).Value);

                        return TdsParserExtensions.WriteEncodingChar(sch, actualLength, offset, defaultEncoding, stateObj, defaultEncoding, canAccumulate: false);
                    }
                    else
                    {
                        Debug.Assert(value is SqlString);
                        return TdsParserExtensions.WriteEncodingChar(((SqlString)value).Value, actualLength, offset, defaultEncoding, stateObj, defaultEncoding, canAccumulate: false);
                    }
                ...
                case TdsEnums.SQLXMLTYPE:
                    ...
                    if (value is SqlChars)
                    {
                        return TdsParserExtensions.WriteCharArray(((SqlChars)value).Value, actualLength, offset, stateObj, canAccumulate: false);
                    }
                    else
                    {
                        Debug.Assert(value is SqlString);
                        return TdsParserExtensions.WriteString(((SqlString)value).Value, actualLength, offset, stateObj, canAccumulate: false);
                    }

                case TdsEnums.SQLNUMERICN:
                    Debug.Assert(type.FixedLength <= 17, "Decimal length cannot be greater than 17 bytes");
                    TdsParserExtensions.WriteSqlDecimal((SqlDecimal)value, stateObj);
                    break;
                ...
                case TdsEnums.SQLDATETIMN:
                    ...
                    {
                        TdsParserExtensions.WriteInt(dt.DayTicks, stateObj);
                        TdsParserExtensions.WriteInt(dt.TimeTicks, stateObj);
                    }
        ...
        private sealed class TdsOutputStream : Stream
        {
            ...
            public override void Write(byte[] buffer, int offset, int count)
            {
                ...
                if (count > 0)
                {
                    TdsParserExtensions.WriteInt(count, _stateObj); // write length of chunk
            ...
            Encoding defaultEncoding)
        {
            ...
            switch (type.NullableType)
            {
                ...
                case TdsEnums.SQLTEXT:
                    {
                        ...
                        else
                        {
                            if (type.IsPlp)
                            {
                                TdsParserExtensions.WriteInt(encodingByteSize, stateObj);               // chunk length
                            }
                            if (value is byte[])
                ...
                case TdsEnums.SQLXMLTYPE:
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                // convert to cchars instead of cbytes
                                actualLength >>= 1;
                                return TdsParserExtensions.WriteString((string)value, actualLength, offset, stateObj, canAccumulate: false);
                            }
                        }
                    }
                case TdsEnums.SQLNUMERICN:
                ...
                case TdsEnums.SQLDATE:
                    {
                        TdsParserExtensions.WriteDate((DateTime)value, stateObj);
                        break;
                    }
                ...
                case TdsEnums.SQLTIME:
                    ...
                    TdsParserExtensions.WriteTime((TimeSpan)value, scale, actualLength, stateObj);
                    break;
                ...
                case TdsEnums.SQLDATETIME2:
                    ...
                    TdsParserExtensions.WriteDateTime2((DateTime)value, scale, actualLength, stateObj);
                    break;

                case TdsEnums.SQLDATETIMEOFFSET:
                    TdsParserExtensions.WriteDateTimeOffset((DateTimeOffset)value, scale, actualLength, stateObj);
                    break;
        ...
        internal static void WriteParameterVarLen(MetaType type, int size, bool isNull, TdsParserStateObject stateObj, bool unknownLength = false)
        {
            ...
            { // text/image/SQLVariant have a 4 byte length, plp datatypes have 8 byte lengths
                if (isNull)
                {
                    ...
                    {
                        TdsParserExtensions.WriteInt(unchecked((int)TdsEnums.VARLONGNULL), stateObj);
                    }
                }
                ...
                {
                    // Non-xml plp types
                    TdsParserExtensions.WriteLong((long)size, stateObj);
                }
                else
                {
                    TdsParserExtensions.WriteInt(size, stateObj);
                }
            }
